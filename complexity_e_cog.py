...
DL = 5  # Desired lambda
SIGMA = 0.05  # Sigma value for state collapsing
...
def calculate_and_graph_complexities(dataframe):
    """
    Calculate complexities for each column and graph the results.
    """
    plt.figure()
    complexities_dict = {}
    ...
    complexities_df.to_csv("complexity_scores.csv", index=False)

def main():
    if len(sys.argv) != 2:
        print("Usage: python program.py <path_to_csv>")
        sys.exit(1)

    file_path = sys.argv[1]
    dataframe = read_csv_columns(file_path, COLUMNS_TO_PROCESS)
    calculate_and_graph_complexities(dataframe)
